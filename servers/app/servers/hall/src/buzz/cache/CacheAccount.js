...
exports.getAccountFieldByIdSync = getAccountFieldByIdSync;
exports.getAccountsFieldsByIdsSync = getAccountsFieldsByIdsSync;
...
async function setTest(uid, value) {
    let account = await getAccountFieldByIdSync(uid,['test']);
    account.test = value;
    account.commit();
}
...
function setWeaponSkin(account, cur, cb) {
    ...
    if (account && cur) {
        ...
        //统计武器皮肤dfc
        let mission = new RewardModel(account);
        let num = weapon_skin.own.length;
...
function getAccountFieldByIdSync(id, fields) {
    fields = fields || ['platform'];
...
async function getAccountsFieldsByIdsSync(ids, fields) {
    fields = fields || ['platform'];
    if (!(ids instanceof Array) || ids.length === 0) {
        ...
        for (let i = 0; i < ids.length; i++) {
            try {
                let account = await redisAccountSync.getAccountAsync(ids[i], fields);
