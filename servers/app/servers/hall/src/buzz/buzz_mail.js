...
function _sendMail(dataObj, cb) {
    ...
    DaoMail.sendMail(dataObj, function (err, insertId) {
        ...
        if (type == MAIL_TYPE.SYS) {
            DaoMail.addMailForAll(mysqlConnector, mid, cb);
            CacheAccount.addSysMail(mid);
        }
        else if (type == MAIL_TYPE.SPECIFY) {
            DaoMail.addMailForPlayer(mysqlConnector, mid, player_list, cb);
