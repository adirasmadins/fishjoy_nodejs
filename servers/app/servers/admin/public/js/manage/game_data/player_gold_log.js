...
$(function(){
	...
	$('body').on('click','#search_btn',function(){
		...
		$(this).attr({'data-sday':sday,'data-eday':eday,'data-uid':uids,'data-sceneid':sceneId});
		var data = sceneId ? {'startDate':sday,'endDate':eday,'uid':uids,'start':1,'length':actions.listnum,'sceneId':sceneId} : {'startDate':sday,'endDate':eday,'uid':uids,'start':1,'length':actions.listnum};
		if(sday && eday){
	...
	$('body').on('click','.pagination a[data-page]',function(){
		...
		var parm = sceneId ? {'startDate':data.sday,'endDate':data.eday,'uid':uids,'start':data.startpage,'length':actions.listnum,'sceneId':sceneId} : {'startDate':data.sday,'endDate':data.eday,'uid':uids,'start':data.startpage,'length':actions.listnum};
		console.log(parm);
