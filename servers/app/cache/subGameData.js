...
class SubGameData {
    ...
    listen(cb) {
        ...
        redisClient.sub(redisKey.DATA_EVENT_SYNC.PUMPWATER, this._pumpwater.bind(this));
        redisClient.sub(redisKey.DATA_EVENT_SYNC.PLATFORM_CATCH_REVISE, this._platform_catch_revise.bind(this));
        redisClient.sub(redisKey.DATA_EVENT_SYNC.PLATFORM_CATCHRATE, this._platform_catchrate.bind(this));
        redisClient.sub(redisKey.DATA_EVENT_SYNC.SCENE_CATCHRATE, this._scene_catchrate.bind(this));
        redisClient.sub(redisKey.DATA_EVENT_SYNC.PLAYER_CATCH_RATE, this._player_catchrate.bind(this));
        redisClient.sub(redisKey.DATA_EVENT_SYNC.PLAYER_GAIN_LOSS_LIMIT, this._player_gain_loss_limit.bind(this));
        redisClient.sub(redisKey.DATA_EVENT_SYNC.CASH_RECHAREGE_PERCET, this._cash_recharege_percet.bind(this));
    }
    ...
    _player_catchrate(value) {
        ...
        if (catchRate >= range_player[0] && catchRate <= range_player[1]) {
            //TODO::
            playerEvent.emit(value.uid, ACCOUNTKEY.PLAYER_CATCH_RATE, catchRate);
        } else {
...
module.exports = new SubGameData();